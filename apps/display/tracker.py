...
def intercept(chs, tgt):
    ...
    :ret: Adjustment to present heading as an angle
    '''
    p_chs, v_chs = chs
    p_tgt, v_tgt = tgt
...
def _intercept(chs, tgt):
    ...
    if check_region(tgt_head, to_tgt_head, chs_head):
        ...
        icept = intercept_position(chs, tgt)
        if icept:
            return chs_head - (icept - p_chs).heading
        else:
            # if no intercept, turn torwards the target
            return chs_head - to_tgt_head
            # h = (360 - v_chs.heading.value + (v_tgt + to_tgt).heading.value) % 360
            # return Heading(h)
    else:
        # If outside region, turn towards chase region
        h = (360 - v_chs.heading.value + (v_tgt + to_tgt).heading.value) % 360
        return Heading(h)
        # return chs_head - to_tgt_head
...
def intercept_position(chs, tgt):
    p_chs, v_chs = chs
    p_tgt, v_tgt = tgt
    ...
    if (v_tgt.lon != v_chs.lon):
        t1 = round((p_chs.lon - p_tgt.lon).value/(v_tgt.lon - v_chs.lon)*60)
    if (v_tgt.lat != v_chs.lat):
        t2 = round((p_chs.lat - p_tgt.lat).value/(v_tgt.lat - v_chs.lat)*60)

    if all([t1, t1 > 0, not t2 or t1 == t2]):
        return Position(p_chs.lat.value + v_chs.lat * t1/60,
                        p_chs.lon.value + v_chs.lon * t1/60)
    elif all([not t1, t2, t2 > 0]):
        return Position(p_chs.lat.value + v_chs.lat * t2/60,
                        p_chs.lon.value + v_chs.lon * t2/60)
    return None
