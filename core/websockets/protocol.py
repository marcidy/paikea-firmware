...
def urlparse(uri):
    """ Parse ws:// URIs
        ...
        :param str url: a valid websocket uri
        :rtype: namedtuple
        :return: URI named tuple with ('hostname', 'port', 'path')
...
class Websocket:
    ...
    async def read_frame(self, max_size=None):
        ''' Read a websocket frame from underlying stream

            :rtype: tuple
            :return: (close indicator, opcode, data)
    ...
    def write_frame(self, opcode, data=b''):
        ''' Write a websocket frame to underlying stream.

            :param bytes opcode: Websocket opcode for frame
            :param bytes data: bytes to send in frame
    ...
    async def recv(self):
        ''' Receive frames from underlying stream. '''
        while self.open:
            try:
    ...
    async def send(self, buf):
        ''' Send a buffer of bytes over the websocket '''
        if not self.open:
            return
    ...
    async def wait_closed(self):
        ''' Wait for underlying stream to close. '''
        await self._stream.drain()
        await self._stream.wait_closed()
        self.close()

    def close(self, code=CLOSE_OK, reason=''):
        ''' Close the websocket with given code and reason.

            :param code: Code for reason to close websocket
            :param str reason: Reason for closing websocket
